...
# Choose the IC analysis subregion to use for the plot

ranges = 'outer'

# plot info for distance binned average charge plot!

plot_info = {'title': 'Scaled absolute charge observed for all IC DOMs (weighted)\n Inner IC analysis region\n', 'xlabel': 'Track to DOM distance (m)', 'ylabel': 'Scaled absolute charge', 'ofile': 'scaled_av_charge_string_region.pdf'}
...
def process(dataset_path,weights_IC,ranges):
    ...
    strings = infile.root.StringCut.cols.item[:]
    # The variable lenght is used totransform a per event variable to a per DOM variable
    length_IC = infile.root.lenghtSPE.cols.value[:] # One per event, so it can be used to probe number of events
    infile.close()

    # In this script we use string number to separate the data!
    
    charges, weights, reco_distance = data_in_range(total_charge_IC, weights_IC, reco_distance_IC, strings, ranges)
    
    dataset['num_events'] = len(charges)
...
def process_exp(dataset_paths,ranges):
    ...
    for dataset_path in dataset_paths: 
        ...
        strings.extend(infile.root.StringCut.cols.item[:])
    ...
    charges, reco_distance = data_in_range(total_charge_IC, None, reco_distance_IC, strings, ranges)
...
def data_in_range(data, weights, reco_distance, variable, ranges):
    ...
    inner_strings = np.array([26,27,37,46,45,35])
    outer_strings = np.array([17,18,19,28,38,47,56,55,54,44,34,25])

    if (ranges == 'inner'):
        cut = np.in1d(variable,inner_strings)

    else:
        cut = np.in1d(variable,outer_strings)
...
def plot_distributions(data, info, args):  #, info, args):
    ...
    plt.axvline(20,linestyle = '--', color = 'r')
...
def main():
    ...
    parser.add_argument('-l', '--labels', help='Labels for the MC and data plots', type = str,
                    nargs = '+',  default = ['2015 experimental data','MC 0.891 DOM eff','MC 0.990 DOM eff',
                                             'MC 1.089 nominal DOM eff','MC 1.188 nominal DOM eff'])
    ...
    for i in range(0,len(args.sim)):
        ...
        charge_info_sim['label'] = args.labels[i+1] + ' ({} entries)'.format(simdata['num_events'])
    ...
    if (ranges != 'inner'):
        plot_info['title'] = 'Scaled absolute charge observed for all IC DOMs (weighted)\n Outer IC analysis region\n' 

    plot_distributions(data,plot_info,args) 

    plot_info['ylabel'] = 'Average charge (PE)'
    plot_info['ofile'] = 'unscaled_av_charge_z_endpoint.pdf'
