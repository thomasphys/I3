...
from general import get_truth_muon, get_truth_endpoint, count_hits, reco_endpoint, move_cut_variables, movellhparams, tot_charge
...
#    options['partitions'] = 5
    ...
    ### Commented out for MC ( prescale MinBias == 1 for MC, also no NullSplit0_filterMask in MC ) 
    ...
    # Generate RTTWOfflinePulses_FR_WIMP, used to generate the finiteReco reconstruction in data
...
#    
    ...
    tray.AddSegment(FiniteReco,'FiniteReco',
                    suffix = 'DOMeff',
                    Pulses = options['pulses_name']) #TODO use RTTWOfflinePulses_FR_WIMP instead!!!

    tray.AddModule(FiniteRecoFilter, 'FiniteRecoFilter')

    tray.AddModule(TruthMuon, 'TruthMuon')
#    tray.AddModule(truth_info, 'truth_info')
...
#                   endpoint_fit='TruthMuon')

    #Moving LLH parameters for finite reco cuts    
   ...
   #                  OutputI3DirectHitsValuesBaseName='MPEFitDOMeffDirectHits')
    ...
    tray.AddModule(move_cut_variables, 'move_cut_variables',
        ...
        #             direct_hits_name='SplineM*PEDirectHits',
        #             fit_params_name='SplineMPEFitParams')
